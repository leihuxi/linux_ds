...
rb_insert_augmented_cached(struct rb_node *node, struct rb_root_cached *root,
			   ...
			   const struct rb_augment_callbacks *augment)
{
	__rb_insert_augmented(node, &root->rb_root, newleft, &root->rb_leftmost,
			      augment->rotate);
}
		   ...
		   const struct rb_augment_callbacks *augment)
{
	struct rb_node *rebalance =
		__rb_erase_augmented(node, root, NULL, augment);
	if (rebalance)
...
rb_erase_augmented_cached(struct rb_node *node, struct rb_root_cached *root,
			  const struct rb_augment_callbacks *augment)
{
	struct rb_node *rebalance = __rb_erase_augmented(
		node, &root->rb_root, &root->rb_leftmost, augment);
	if (rebalance)
